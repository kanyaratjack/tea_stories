...
typedef BackendSyncOrderSnapshot = ({
  String orderNo,
  String orderType,
  String orderChannel,
  double total,
...
typedef BackendSyncRefundSnapshot = ({
  double amount,
  String reason,
  DateTime createdAt,
});
typedef SyncQueueTask = ({
  int id,
  String taskType,
  String taskKey,
...
class PosRepository {
  ...
  Future<CheckoutResult> createOrder({
    ...
    final platformOrderIdCode = row.isNotEmpty
        ...
        : '';
    final createdAtCode = row.isNotEmpty
        ? DateTime.parse(
            (row.first['created_at'] as String?) ?? now.toIso8601String(),
          )
        : now;
  ...
  Future<List<SyncQueueTask>> fetchDueSyncTasks({int limit = 20}) async {
    final now = DateTime.now().toIso8601String();
    final rows = await db.query(
      'sync_queue',
      columns: ['id', 'task_type', 'task_key', 'payload_json', 'attempt_count'],
      where: 'next_retry_at <= ?',
      whereArgs: [now],
      orderBy: 'datetime(next_retry_at) ASC, id ASC',
      limit: limit,
    );
    return rows
        .map((row) {
  ...
  Future<void> markSyncTaskSuccess(int id) async {
  ...
  Future<void> markSyncTaskFailed(
    ...
    final attempts = (row.first['attempt_count'] as num?)?.toInt() ?? 0;
    final nextAttempts = attempts + 1;
    final delaySeconds = _retryDelaySeconds(nextAttempts);
  ...
  Future<void> deleteSyncTaskByKey(String taskKey) async {
    await db.delete(
      'sync_queue',
      where: 'task_key = ?',
      whereArgs: [taskKey.trim()],
    );
  }

  Future<int> countPendingSyncTasks() async {
    final rows = await db.rawQuery('SELECT COUNT(*) AS c FROM sync_queue');
    return (rows.first['c'] as num?)?.toInt() ?? 0;
  }

  Future<String?> fetchLatestSyncQueueError() async {
    final rows = await db.query(
      'sync_queue',
      columns: ['last_error'],
    ...
    final error = (rows.first['last_error'] as String?)?.trim() ?? '';
    return error.isEmpty ? null : error;
  }

  Future<List<BackendSyncOrderSnapshot>> fetchOrdersForBackendSync({
    DateTime? startAtInclusive,
  }) async {
    final rows = await db.rawQuery(
      '''
      SELECT
        order_no,
        COALESCE(order_type, 'inStore') AS order_type,
        COALESCE(order_channel, '') AS order_channel,
        total,
        created_at
      FROM orders
      WHERE status IN (?, ?, ?)
        ${startAtInclusive == null ? '' : 'AND created_at >= ?'}
      ...
      [
        ...
        'partially_refunded',
        'refunded',
        if (startAtInclusive != null) startAtInclusive.toIso8601String(),
      ],
    );
    return rows
        .map(
          (row) => (
            ...
            orderType: (row['order_type'] as String?) ?? 'inStore',
            orderChannel: (row['order_channel'] as String?) ?? '',
  ...
  Future<List<BackendSyncRefundSnapshot>> fetchRefundsForBackendSync(
    String orderNo,
  ) async {
    final rows = await db.query(
      'refunds',
      columns: ['amount', 'reason', 'created_at'],
      where: 'order_no = ?',
      whereArgs: [orderNo.trim()],
  ...
  int _retryDelaySeconds(int attemptCount) {
    ...
    final exp = attemptCount.clamp(1, 6);
    final value = 5 * (1 << (exp - 1));

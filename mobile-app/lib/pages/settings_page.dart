...
class _PrinterNetworkSettingsPageState
  ...
  late final TextEditingController _labelPrinterIpController;
  late final TextEditingController _uploadApiController;
  late final TextEditingController _posApiController;
  ReceiptPrintMode _receiptPrintMode = ReceiptPrintMode.bitmap;
  int _bottomFeedLinesBeforeCut = 6;
  int _labelBottomFeedLinesBeforeCut = 6;
  ...
  bool _isTestingPosBackend = false;
  bool _isSyncingHistory = false;
  bool _isRetryingSyncQueue = false;
  ...
  void initState() {
    ...
    _labelPrinterIpController = TextEditingController();
    _uploadApiController = TextEditingController();
    _posApiController = TextEditingController();
    _load();
  }
  ...
  Future<void> _load() async {
    ...
    final posApi = await store.loadPosApiBaseUrl() ?? '';
          ...
          children: [
            ...
            Wrap(
              ...
              children: [
                ...
                OutlinedButton.icon(
                  onPressed: _isTestingPosBackend ? null : _testPosBackend,
                  icon: _isTestingPosBackend
                      ? const SizedBox(
                          width: 14,
                          height: 14,
                          child: CircularProgressIndicator(strokeWidth: 2),
                  ...
                  label: Text(
                    t('测试POS后端', 'ทดสอบ POS Backend', 'Test POS Backend'),
                  ),
                ),
                OutlinedButton.icon(
                  onPressed: _isSyncingHistory ? null : _openHistorySyncSheet,
                  icon: _isSyncingHistory
                      ? const SizedBox(
                          width: 14,
                          height: 14,
                          child: CircularProgressIndicator(strokeWidth: 2),
                  ...
                  label: Text(
                    t('补同步历史订单', 'ซิงก์ออเดอร์ย้อนหลัง', 'Sync History'),
                  ),
                ),
                OutlinedButton.icon(
  ...
  Future<void> _save(AppI18n i18n) async {
    ...
    try {
      ...
      await store.savePosApiBaseUrl(_posApiController.text.trim());
      await store.saveDeliveryChannels(_deliveryChannels);
      await widget.controller.refreshBackendSyncConfig();
      if (!mounted) return;
  ...
  Future<void> _testPosBackend() async {
    final baseUrl = _posApiController.text.trim();
    if (baseUrl.isEmpty) {
      showLatestSnackBar(
        context,
        t(
          '请先填写POS后端地址',
          'กรอก URL ของ POS Backend ก่อน',
          'Please set POS backend URL first',
        ),
      );
      return;
    }
    setState(() => _isTestingPosBackend = true);
    try {
      const service = PosBackendSyncService();
      await service.testConnection(apiBaseUrl: baseUrl);
      if (!mounted) return;
      showLatestSnackBar(
        context,
        t('POS后端连接成功', 'เชื่อมต่อ POS Backend สำเร็จ', 'POS backend connected'),
      );
    } catch (e) {
      if (!mounted) return;
      showLatestSnackBar(
        context,
        '${t('POS后端连接失败', 'เชื่อมต่อ POS Backend ล้มเหลว', 'POS backend connection failed')}: $e',
      );
    } finally {
      if (mounted) {
        setState(() => _isTestingPosBackend = false);
      }
    }
  }

  Future<void> _openHistorySyncSheet() async {
          ...
          child: ListView(
            ...
            children: [
              ...
              ListTile(
                ...
                title: Text(
                  t('同步最近30天', 'ซิงก์ 30 วันล่าสุด', 'Sync recent 30 days'),
                ),
    ...
    await _syncHistory(days: choice == 0 ? null : choice);
  }

  Future<void> _syncHistory({int? days}) async {
    setState(() => _isSyncingHistory = true);
    try {
      final report = await widget.controller.syncHistoricalOrdersToBackend(
        days: days,
      );
      if (!mounted) return;
      final scopeText = days == null
          ? t('全部', 'ทั้งหมด', 'all')
          : t('最近$days天', '$days วันล่าสุด', 'last $days days');
      showLatestSnackBar(
        context,
        t(
          '历史同步完成（$scopeText）：订单 ${report.syncedOrders}/${report.totalOrders}，已存在 ${report.skippedExistingOrders}，失败 ${report.failedOrders}；退款 ${report.syncedRefunds}/${report.totalRefunds}，失败 ${report.failedRefunds}',
          'ซิงก์ย้อนหลังเสร็จแล้ว ($scopeText): ออเดอร์ ${report.syncedOrders}/${report.totalOrders}, มีอยู่แล้ว ${report.skippedExistingOrders}, ล้มเหลว ${report.failedOrders}; คืนเงิน ${report.syncedRefunds}/${report.totalRefunds}, ล้มเหลว ${report.failedRefunds}',
          'History sync done ($scopeText): orders ${report.syncedOrders}/${report.totalOrders}, existing ${report.skippedExistingOrders}, failed ${report.failedOrders}; refunds ${report.syncedRefunds}/${report.totalRefunds}, failed ${report.failedRefunds}',
        ),
      ...
      showLatestSnackBar(
        context,
        '${t('历史同步失败', 'ซิงก์ย้อนหลังล้มเหลว', 'History sync failed')}: $e',
      );
    } finally {
      if (mounted) {
        setState(() => _isSyncingHistory = false);
      }
    }
  }
  ...
  Widget _buildSyncStatusPanel() {
    return AnimatedBuilder(
      ...
      builder: (context, _) {
        final pending = widget.controller.pendingSyncTaskCount;
        final latestError = widget.controller.latestSyncError;
        final lastAttempt = widget.controller.lastSyncAttemptAt;
        final lastSuccess = widget.controller.lastSyncSuccessAt;
        final networkText = widget.controller.isNetworkReachable
            ? t('在线', 'ออนไลน์', 'Online')
            : t('离线', 'ออฟไลน์', 'Offline');
        final networkColor = widget.controller.isNetworkReachable
          ...
          child: Column(
            ...
            children: [
              Row(
                children: [
                  Text(
                    t('同步状态', 'สถานะการซิงก์', 'Sync Status'),
                    style: const TextStyle(fontWeight: FontWeight.w700),
                  ),
                  const Spacer(),
                  Text(
              ...
              const SizedBox(height: 8),
              Text('${t('待同步', 'รอซิงก์', 'Pending')}: $pending'),
              Text(
                '${t('最近尝试', 'ลองล่าสุด', 'Last Attempt')}: ${_formatTime(lastAttempt)}',
              ),
              Text(
                '${t('最近成功', 'สำเร็จล่าสุด', 'Last Success')}: ${_formatTime(lastSuccess)}',
              ),
              Text(
                '${t('最近错误', 'ข้อผิดพลาดล่าสุด', 'Latest Error')}: ${latestError == null || latestError.trim().isEmpty ? t('无', 'ไม่มี', 'None') : latestError}',
                maxLines: 2,
                overflow: TextOverflow.ellipsis,
                style: TextStyle(
                  color: latestError == null || latestError.trim().isEmpty
                      ? null
                      : const Color(0xFFC62828),
                ),
              ),
              ...
              Align(
                ...
                child: OutlinedButton.icon(
                  onPressed: _isRetryingSyncQueue ? null : _retrySyncQueueNow,
                  icon: _isRetryingSyncQueue
                      ? const SizedBox(
                          width: 14,
                          height: 14,
                          child: CircularProgressIndicator(strokeWidth: 2),
                  ...
                  label: Text(t('立即重试同步', 'ลองซิงก์ทันที', 'Retry Sync Now')),
                ),
              ),
            ],
          ),
  ...
  String _formatTime(DateTime? value) {
    if (value == null) return t('未发生', 'ยังไม่มี', 'Never');
  ...
  Future<void> _retrySyncQueueNow() async {
    setState(() => _isRetryingSyncQueue = true);
    try {
      await widget.controller.retrySyncQueueNow();
      if (!mounted) return;
      showLatestSnackBar(
        context,
        t('已触发同步重试', 'เริ่มลองซิงก์แล้ว', 'Sync retry started'),
      );
    } catch (e) {
      if (!mounted) return;
      showLatestSnackBar(
        context,
        '${t('同步重试失败', 'ลองซิงก์ล้มเหลว', 'Sync retry failed')}: $e',
      );
    } finally {
      if (mounted) {
        setState(() => _isRetryingSyncQueue = false);
      }
    }
  }

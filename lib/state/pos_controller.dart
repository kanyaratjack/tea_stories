...
class PosController extends ChangeNotifier {
  ...
  Future<CheckoutResult> checkoutWith({
    ...
    if (method == PaymentMethod.cash) {
      if (cashReceivedAmount == null || cashReceivedAmount < checkoutTotal) {
    ...
    try {
      ...
      final checkoutResult = await repository.createOrder(
        ...
        changeAmount: method == PaymentMethod.cash
            ? ((cashReceivedAmount ?? 0) - checkoutTotal).clamp(
                0,
                double.infinity,
              )
            : null,
  ...
  double estimateCheckoutTotal(OrderType orderType) {
    return _buildPricingForOrderType(orderType).total;
  }

  CheckoutPricingPreview previewPricing(OrderType orderType) {
    final pricing = _buildPricingForOrderType(orderType);
  ...
  Future<double> refundOrder({
    ...
    final amount = await repository.refundOrder(
      orderNo: orderNo,
      reason: reason.trim().isEmpty ? 'manual refund' : reason.trim(),
      managerPin: '',
    );
    _recentPaidOrders = await repository.fetchRecentPaidOrders(
      limit: _ordersFetchLimit,

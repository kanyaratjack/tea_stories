...
class AppSettingsStore {
  ...
  static const _posApiBaseUrlKey = 'pos_api_base_url';
  ...
  Future<String?> loadPosApiBaseUrl() async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final raw = prefs.getString(_posApiBaseUrlKey)?.trim() ?? '';
      if (raw.isNotEmpty) return raw;
      // Fallback: reuse upload backend URL when POS URL is not configured yet.
      final legacy = prefs.getString(_uploadApiBaseUrlKey)?.trim() ?? '';
      return legacy.isEmpty ? null : legacy;
    } catch (e) {
      debugPrint('loadPosApiBaseUrl failed, fallback to null: $e');
      return null;
    }
  }

  Future<void> savePosApiBaseUrl(String url) async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final value = url.trim();
      if (value.isEmpty) {
        await prefs.remove(_posApiBaseUrlKey);
      } else {
        await prefs.setString(_posApiBaseUrlKey, value);
      }
    } catch (e) {
      debugPrint('savePosApiBaseUrl failed, ignore: $e');
    }
  }
